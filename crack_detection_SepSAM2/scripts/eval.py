...
def main():
    ...
    ap.add_argument("--tile", type=int, default=0,
                    help=">0 時改用滑動窗口 CMC（每 tile 跑完整 CMC 再聯集縫合）。"
                         "註：對全圖訓練的 Agent 實測會降低效能，僅在 tile 重訓 Agent 後使用。")
    ap.add_argument("--stride", type=int, default=256, help="滑窗步長（--tile>0 時生效）")
    args = ap.parse_args()
    ...
    for stem in stems:
        ...
        if args.tile > 0:
            pred = cmc_predict_sliding(rgb, agent, predictor, prompt_fn, hp,
